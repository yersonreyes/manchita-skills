...
export class HibridacionAgregacionDataDto {
  ...
  @ApiPropertyOptional()
  @IsOptional()
  @IsIn(['feature-stacking', 'best-of-each', 'plus-minus', 'mashup', ''])
  tecnica?: string;
  @ApiProperty({ type: [IdeaBaseDto] })
  @IsArray()
  @ValidateNested({ each: true })
  @Type(() => IdeaBaseDto)
  ideasBase: IdeaBaseDto[];
  @ApiPropertyOptional({ type: [CombinacionDto] })
  @IsOptional()
  @IsArray()
  @ValidateNested({ each: true })
  @Type(() => CombinacionDto)
...
export class HibridacionAgregacionAnalyzeReqDto {
  ...
  @ApiProperty({ type: HibridacionAgregacionDataDto })
  @ValidateNested()
  @Type(() => HibridacionAgregacionDataDto)
  data: HibridacionAgregacionDataDto;
