...
export class BriefDataDto {
  ...
  @ApiProperty({ type: [String] }) @IsArray() @IsString({ each: true }) objetivosSecundarios: string[];
  @ApiPropertyOptional() @IsOptional() @IsString() usuarioTarget?: string;
  @ApiProperty({ type: [String] }) @IsArray() @IsString({ each: true }) inScope: string[];
  @ApiProperty({ type: [String] }) @IsArray() @IsString({ each: true }) outScope: string[];
  ...
  @ApiProperty({ type: [String] }) @IsArray() @IsString({ each: true }) entregables: string[];
  @ApiProperty({ type: [String] }) @IsArray() @IsString({ each: true }) metricasExito: string[];
  @ApiProperty({ type: [String] }) @IsArray() @IsString({ each: true }) riesgos: string[];
...
export class BriefAnalyzeReqDto {
  ...
  @ApiProperty({ type: BriefDataDto }) @ValidateNested() @Type(() => BriefDataDto) data: BriefDataDto;
