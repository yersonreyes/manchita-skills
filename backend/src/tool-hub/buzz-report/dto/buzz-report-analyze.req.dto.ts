...
export class BuzzReportDataDto {
  ...
  @ApiProperty({ type: [BuzzReportMencionDto] }) @IsArray() @ValidateNested({ each: true }) @Type(() => BuzzReportMencionDto) menciones: BuzzReportMencionDto[];
  @ApiProperty({ type: [String] }) @IsArray() @IsString({ each: true }) temasRecurrentes: string[];
  @ApiProperty({ type: [String] }) @IsArray() @IsString({ each: true }) vocesInfluyentes: string[];
...
export class BuzzReportAnalyzeReqDto {
  ...
  @ApiProperty({ type: BuzzReportDataDto }) @ValidateNested() @Type(() => BuzzReportDataDto) data: BuzzReportDataDto;
