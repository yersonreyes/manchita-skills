...
export class TestCuantitativoDataDto {
  ...
  @ApiPropertyOptional() @IsOptional() @IsNumber() participantes?: number | null;
  @ApiProperty({ type: [TestTareaDataDto] }) @IsArray() @ValidateNested({ each: true }) @Type(() => TestTareaDataDto) tareas: TestTareaDataDto[];
...
export class TestCuantitativoAnalyzeReqDto {
  ...
  @ApiProperty({ type: TestCuantitativoDataDto }) @ValidateNested() @Type(() => TestCuantitativoDataDto) data: TestCuantitativoDataDto;
