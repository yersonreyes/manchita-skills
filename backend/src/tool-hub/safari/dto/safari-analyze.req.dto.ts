...
export class SesionSafariDto {
  ...
  @ApiProperty({ type: [ObservacionDto] }) @IsArray() @ValidateNested({ each: true }) @Type(() => ObservacionDto) observaciones: ObservacionDto[];
...
export class SafariDataDto {
  ...
  @ApiProperty({ type: [SesionSafariDto] }) @IsArray() @ValidateNested({ each: true }) @Type(() => SesionSafariDto) sesiones: SesionSafariDto[];
...
export class SafariAnalyzeReqDto {
  ...
  @ApiProperty({ type: SafariDataDto }) @ValidateNested() @Type(() => SafariDataDto) data: SafariDataDto;
