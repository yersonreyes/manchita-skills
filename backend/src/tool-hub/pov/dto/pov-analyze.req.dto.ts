...
export class PovDataDto {
  ...
  @ApiProperty({ type: [PovItemDto] }) @IsArray() @ValidateNested({ each: true }) @Type(() => PovItemDto) povs: PovItemDto[];
}

export class PovAnalyzeReqDto {
  @ApiProperty() @IsNumber() toolApplicationId: number;
  @ApiProperty({ type: PovDataDto }) @ValidateNested() @Type(() => PovDataDto) data: PovDataDto;
