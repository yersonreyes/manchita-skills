...
export class HibridacionSintesisDataDto {
  ...
  @ApiProperty({ type: [ConceptoBaseDto] }) @IsArray() @ValidateNested({ each: true }) @Type(() => ConceptoBaseDto) conceptosBase: ConceptoBaseDto[];
  @ApiPropertyOptional({ type: [String] }) @IsOptional() @IsArray() @IsString({ each: true }) puntosConexion?: string[];
  @ApiPropertyOptional() @IsOptional() @IsIn(['superficial', 'estructural', 'paradigmatico', '']) nivelSintesis?: string;
...
export class HibridacionSintesisAnalyzeReqDto {
  ...
  @ApiProperty({ type: HibridacionSintesisDataDto }) @ValidateNested() @Type(() => HibridacionSintesisDataDto) data: HibridacionSintesisDataDto;
