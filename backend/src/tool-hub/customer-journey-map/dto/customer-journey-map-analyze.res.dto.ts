...
export class CjmEtapaAnalisisDto {
  ...
  @ApiProperty({ example: 'El usuario llega con expectativas altas pero poca información.' })
  insight: string;
}

export class CustomerJourneyMapReportDto {
  @ApiProperty({ example: 'El journey revela una experiencia con picos emocionales...' })
  executiveSummary: string;

  @ApiProperty({ type: [String], example: ['En el checkout el usuario decide abandonar o comprar'] })
  ...
  @ApiProperty({ type: [String], example: ['Simplificar el onboarding en 3 pasos'] })
  oportunidadesPriorizadas: string[];

  @ApiProperty({ type: [String], example: ['Implementar progress bar en checkout'] })
