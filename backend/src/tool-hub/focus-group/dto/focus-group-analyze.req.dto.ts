...
export class FocusGroupDataDto {
  ...
  @ApiPropertyOptional() @IsOptional() @IsString() cantidadParticipantes?: string;
  @ApiPropertyOptional() @IsOptional() @IsString() ubicacion?: string;
  @ApiPropertyOptional() @IsOptional() @IsString() fecha?: string;
  @ApiProperty({ type: [FocusGroupPreguntaDto] }) @IsArray() @ValidateNested({ each: true }) @Type(() => FocusGroupPreguntaDto) preguntas: FocusGroupPreguntaDto[];
  @ApiPropertyOptional() @IsOptional() @IsString() dinamicasGrupales?: string;
  @ApiProperty({ type: [String] }) @IsArray() @IsString({ each: true }) citasClave: string[];
...
export class FocusGroupAnalyzeReqDto {
  ...
  @ApiProperty({ type: FocusGroupDataDto }) @ValidateNested() @Type(() => FocusGroupDataDto) data: FocusGroupDataDto;
