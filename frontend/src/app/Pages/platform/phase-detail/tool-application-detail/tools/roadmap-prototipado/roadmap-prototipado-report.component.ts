...
import { RoadmapPrototipadoReportVersionDto } from './roadmap-prototipado.types';

@Component({
  selector: 'app-roadmap-prototipado-report',
  standalone: true,
  imports: [DatePipe],
  template: `
    @if (versions().length === 0) {
      <div class="report-empty">
        <i class="pi pi-calendar" style="font-size:2rem;color:#f59e0b;margin-bottom:.75rem;"></i>
        <p>Definí el contexto y al menos una fase con prototipos, luego presioná <strong>Generar análisis</strong>.</p>
      </div>
    } @else {
      @for (v of versions(); track v.version) {
        <div class="report-version">
          ...
          <div class="report-section">
            <h4 class="report-section__title">Evaluación de la estrategia</h4>
            <p class="report-section__text">{{ v.report.evaluacionEstrategia }}</p>
          </div>

          <div class="report-section">
            <h4 class="report-section__title">Prioridad recomendada</h4>
            <p class="report-section__text">{{ v.report.prioridadRecomendada }}</p>
          </div>

          <div class="report-row">
            <div class="report-section report-section--half">
              <h4 class="report-section__title">Riesgos del timeline</h4>
              <ul class="report-list">
                @for (r of v.report.riesgosTimeline; track r) {
                  <li class="report-list__item report-list__item--red">{{ r }}</li>
                }
              </ul>
            </div>
            <div class="report-section report-section--half">
              <h4 class="report-section__title">Bottlenecks identificados</h4>
              <ul class="report-list">
                @for (b of v.report.bottlenecks; track b) {
                  <li class="report-list__item report-list__item--amber">{{ b }}</li>
                }
              </ul>
            </div>
          </div>
  ...
  styles: [`
    ...
    .report-section--summary { background: #fef3c7; border-left: 4px solid #f59e0b; }
    ...
    .report-list__item--amber { background: #fffbeb; border-left-color: #f59e0b; }
...
export class RoadmapPrototipadoReportComponent {
  versions = input<RoadmapPrototipadoReportVersionDto[]>([]);
}
