import { Component, computed, input, signal } from '@angular/core';
import { DatePipe } from '@angular/common';
import { MapaEvolucionReportVersionDto } from './mapa-evolucion-innovacion.types';

@Component({
  selector: 'app-mapa-evolucion-innovacion-report',
  standalone: true,
  imports: [DatePipe],
  template: `
    @if (reports().length === 0) {
      <div class="empty-report">
        <i class="pi pi-sparkles"></i>
        <p>Aún no hay informes generados. Mapeá al menos 2 eras con hitos y hacé clic en "Analizar".</p>
      </div>
    } @else {

      @if (reports().length > 1) {
        <div class="version-selector">
          @for (r of reports(); track r.version; let i = $index) {
            <button
              class="version-btn"
              [class.version-btn--active]="selectedIndex() === i"
              (click)="selectedIndex.set(i)"
            >
              v{{ r.version }} — {{ r.generatedAt | date:'dd/MM/yy HH:mm' }}
            </button>
          }
        </div>
      }

      @if (current()) {
        <div class="report">

          <div class="report__summary">
            <p>{{ current()!.report.executiveSummary }}</p>
          </div>

          @if (current()!.report.analisisPorEra.length) {
            <div class="report__section">
              <p class="report__label">
                <i class="pi pi-history"></i>
                Análisis por era
              </p>
              <div class="report__eras">
                @for (era of current()!.report.analisisPorEra; track $index) {
                  <div class="report__era">
                    <div class="report__era-header">
                      <span class="report__era-name">{{ era.era }}</span>
                      @if (era.periodo) {
                        <span class="report__era-periodo">{{ era.periodo }}</span>
                      }
                    </div>
                    <p class="report__era-patron"><strong>Patrón:</strong> {{ era.patronInnovacion }}</p>
                    <p class="report__era-relevancia">{{ era.relevanciaActual }}</p>
                  </div>
                }
              </div>
            </div>
          }

          @if (current()!.report.patronesEvolutivos.length) {
            <div class="report__section">
              <p class="report__label">
                <i class="pi pi-arrows-h"></i>
                Patrones evolutivos
              </p>
              <ul class="report__list">
                @for (item of current()!.report.patronesEvolutivos; track $index) {
                  <li>{{ item }}</li>
                }
              </ul>
            </div>
          }

          @if (current()!.report.puntosInflexionCriticos.length) {
            <div class="report__section">
              <p class="report__label">
                <i class="pi pi-bolt"></i>
                Puntos de inflexión críticos
              </p>
              <ul class="report__list">
                @for (item of current()!.report.puntosInflexionCriticos; track $index) {
                  <li>{{ item }}</li>
                }
              </ul>
            </div>
          }

          @if (current()!.report.gapsIdentificados.length) {
            <div class="report__section">
              <p class="report__label">
                <i class="pi pi-minus-circle"></i>
                Gaps identificados
              </p>
              <ul class="report__list">
                @for (item of current()!.report.gapsIdentificados; track $index) {
                  <li>{{ item }}</li>
                }
              </ul>
            </div>
          }

          @if (current()!.report.oportunidadesDeInnovacion.length) {
            <div class="report__section">
              <p class="report__label">
                <i class="pi pi-lightbulb"></i>
                Oportunidades de innovación
              </p>
              <ul class="report__list">
                @for (item of current()!.report.oportunidadesDeInnovacion; track $index) {
                  <li>{{ item }}</li>
                }
              </ul>
            </div>
          }

          @if (current()!.report.recommendations.length) {
            <div class="report__section">
              <p class="report__label">
                <i class="pi pi-check-square"></i>
                Recomendaciones
              </p>
              <ul class="report__list report__list--recommendations">
                @for (item of current()!.report.recommendations; track $index) {
                  <li>{{ item }}</li>
                }
              </ul>
            </div>
          }

        </div>
      }
    }
  `,
  styles: [`
    .empty-report {
      display: flex; flex-direction: column; align-items: center;
      gap: 0.5rem; padding: 2rem; color: #9ca3af; text-align: center;
    }
    .empty-report i { font-size: 1.5rem; color: #d1d5db; }
    .empty-report p { font-size: 0.875rem; margin: 0; }

    .version-selector { display: flex; flex-wrap: wrap; gap: 6px; margin-bottom: 12px; }

    .version-btn {
      padding: 4px 10px; border-radius: 6px;
      border: 1px solid var(--p-surface-300); background: transparent;
      font-size: 0.75rem; color: var(--p-text-secondary-color);
      cursor: pointer; transition: all 0.15s;
    }
    .version-btn:hover { background: var(--p-surface-100); }
    .version-btn--active { background: #f0f9ff; border-color: #bae6fd; color: #0369a1; font-weight: 600; }

    .report {
      display: flex; flex-direction: column; gap: 0;
      border: 1px solid #bae6fd; border-radius: 12px;
      background: linear-gradient(135deg, #f0f9ff, #f8fbff); overflow: hidden;
    }

    .report__summary { padding: 14px 16px; border-bottom: 1px solid #bae6fd; }
    .report__summary p { margin: 0; font-size: 0.875rem; color: #374151; line-height: 1.65; }

    .report__section { padding: 12px 16px; border-bottom: 1px solid #bae6fd; }
    .report__section:last-child { border-bottom: none; }

    .report__label {
      display: flex; align-items: center; gap: 6px;
      font-size: 0.6875rem; font-weight: 700;
      letter-spacing: 0.06em; text-transform: uppercase;
      color: #0369a1; margin: 0 0 8px;
    }
    .report__label .pi { font-size: 0.7rem; }

    .report__eras { display: flex; flex-direction: column; gap: 6px; }

    .report__era {
      padding: 8px 10px; border-radius: 8px;
      background: rgba(255,255,255,0.7); border: 1px solid #bae6fd;
    }

    .report__era-header {
      display: flex; align-items: center; gap: 8px; margin-bottom: 4px;
    }

    .report__era-name {
      font-size: 0.8rem; font-weight: 800; color: #0369a1;
    }

    .report__era-periodo {
      font-size: 0.72rem; color: #7dd3fc;
    }

    .report__era-patron {
      margin: 0 0 3px; font-size: 0.8rem; color: #374151; line-height: 1.4;
    }

    .report__era-relevancia {
      margin: 0; font-size: 0.78rem; color: #6b7280; line-height: 1.4; font-style: italic;
    }

    .report__list {
      margin: 0; padding-left: 1.25rem;
      display: flex; flex-direction: column; gap: 5px;
    }
    .report__list li { font-size: 0.8125rem; color: #374151; line-height: 1.5; }
    .report__list--recommendations li { font-weight: 500; color: #1f2937; }
  `],
})
export class MapaEvolucionInnovacionReportComponent {
  reports = input<MapaEvolucionReportVersionDto[]>([]);
  selectedIndex = signal(0);
  current = computed(() => this.reports()[this.selectedIndex()] ?? null);
}
